<script lang="ts">
    import { Flower, Wind, Sparkles, Sun, Moon, Mountain, Clock, Users, ShieldCheck, Star, Heart, ArrowRight, Sunrise, CloudSun, TreePine, Leaf } from 'lucide-svelte';

    let scrollY = $state(0);
</script>

<svelte:window bind:scrollY />

<svelte:head>
    <title>Yoga in Quota | Hotel du Soleil</title>
    <meta name="description" content="Yoga in quota a Torgnon, Valle d'Aosta. Sessioni all'alba e al tramonto a 1.500 metri, meditazione alpina, pranayama e retreats settimanali con vista sul Cervino." />
</svelte:head>

<!-- ═══════════════════ HERO ═══════════════════ -->
<header class="relative h-[85vh] w-full overflow-hidden bg-[#1a1a1a]">
    <div
        class="absolute inset-0 scale-110"
        style="transform: translateY({scrollY * 0.25}px) scale(1.1);"
    >
        <img src="https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?q=80&w=2000&auto=format&fit=crop" class="h-full w-full object-cover opacity-50" alt="Yoga in montagna con vista sulle Alpi" />
        <div class="absolute inset-0 bg-linear-to-b from-black/80 via-black/30 to-alpine-bg"></div>
    </div>

    <div class="absolute inset-0 z-10 flex flex-col items-center justify-end px-6 pb-28 text-center">
        <span class="mb-4 block text-[10px] font-medium tracking-[0.4em] text-white/80 uppercase md:text-xs fade-up-element">
            Equilibrio Interiore · 1.500m
        </span>
        <h1 class="font-serif text-5xl md:text-8xl leading-tight font-light text-white fade-up-element">
            Yoga in Quota
        </h1>
        <p class="mt-6 text-white/50 text-sm font-light max-w-md fade-up-element">
            Dove il respiro incontra l'aria sottile delle Alpi
        </p>
    </div>
</header>

<!-- ═══════════════════ INTRO ═══════════════════ -->
<section class="bg-alpine-bg px-6 py-28">
    <div class="fade-up-element mx-auto max-w-3xl text-center">
        <Flower class="w-10 h-10 text-alpine-gold mx-auto mb-10" />
        <h3 class="mb-10 font-serif text-3xl leading-snug text-alpine-text md:text-4xl">
            Il respiro si fonde con l'aria sottile dei 1.500 metri.
        </h3>
        <p class="text-alpine-muted leading-relaxed font-light text-sm md:text-base border-t border-alpine-border pt-12 mt-12 mx-auto max-w-xl">
            Lo yoga in montagna è un'esperienza che va oltre la pratica. 
            A 1.500 metri, l'aria è più pura, i suoni della natura sostituiscono il rumore della città 
            e il corpo risponde con una profondità diversa. Le nostre sessioni si svolgono 
            all'alba e al tramonto, nella cornice delle Alpi valdostane, 
            dove ogni posizione diventa un dialogo tra te e la montagna.
        </p>
    </div>
</section>

<!-- ═══════════════════ L'ESPERIENZA ═══════════════════ -->
<section class="bg-alpine-bg pb-32 px-6 max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20 items-center">
    <div class="fade-up-element overflow-hidden bg-alpine-border shadow-xl">
        <img src="https://images.unsplash.com/photo-1506126613408-eca07ce68773?q=80&w=1500&auto=format&fit=crop" alt="Yoga all'aperto con vista sulle montagne" class="w-full h-full object-cover img-elegant aspect-4/3 brightness-90" />
    </div>

    <div class="fade-up-element">
        <span class="text-[10px] uppercase tracking-[0.3em] text-alpine-gold font-bold mb-4 block">L'Esperienza</span>
        <h2 class="font-serif text-4xl text-alpine-text mb-6">Pratica tra<br/>Cielo e Terra</h2>
        <p class="text-alpine-muted text-sm font-light leading-relaxed mb-6">
            Le nostre sessioni si tengono sulla terrazza panoramica dell'hotel, affacciata 
            sulla valle e sulle vette del Cervino. In estate, quando il tempo lo permette, 
            ci spostiamo nel prato alpino adiacente, dove l'erba diventa il tuo tappetino 
            e il cielo il tuo soffitto.
        </p>
        <p class="text-alpine-muted text-sm font-light leading-relaxed mb-8">
            La nostra insegnante certificata adatta ogni lezione al gruppo presente: 
            dal principiante assoluto al praticante avanzato. 
            Non serve esperienza, solo la voglia di fermarsi e ascoltarsi.
        </p>
        <div class="grid grid-cols-2 gap-6 pt-6 border-t border-alpine-border">
            <div class="flex items-center gap-3">
                <Heart class="w-5 h-5 text-alpine-gold shrink-0" />
                <span class="text-xs font-bold uppercase tracking-widest text-alpine-text">Tutti i livelli</span>
            </div>
            <div class="flex items-center gap-3">
                <Mountain class="w-5 h-5 text-alpine-gold shrink-0" />
                <span class="text-xs font-bold uppercase tracking-widest text-alpine-text">Vista Cervino</span>
            </div>
        </div>
    </div>
</section>

<!-- ═══════════════════ STATS ═══════════════════ -->
<section class="border-y border-alpine-border bg-white py-20 px-6">
    <div class="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-12 text-center">
        <div class="fade-up-element">
            <p class="font-serif text-4xl md:text-5xl text-alpine-text mb-2">1.500m</p>
            <p class="text-[10px] uppercase tracking-[0.25em] text-alpine-muted font-bold">Di altitudine</p>
        </div>
        <div class="fade-up-element">
            <p class="font-serif text-4xl md:text-5xl text-alpine-text mb-2">60'</p>
            <p class="text-[10px] uppercase tracking-[0.25em] text-alpine-muted font-bold">Per sessione</p>
        </div>
        <div class="fade-up-element">
            <p class="font-serif text-4xl md:text-5xl text-alpine-text mb-2">7</p>
            <p class="text-[10px] uppercase tracking-[0.25em] text-alpine-muted font-bold">Giorni a settimana</p>
        </div>
        <div class="fade-up-element">
            <p class="font-serif text-4xl md:text-5xl text-alpine-text mb-2">8</p>
            <p class="text-[10px] uppercase tracking-[0.25em] text-alpine-muted font-bold">Posti per sessione</p>
        </div>
    </div>
</section>

<!-- ═══════════════════ LE SESSIONI ═══════════════════ -->
<section class="bg-alpine-bg py-32 px-6">
    <div class="max-w-7xl mx-auto">
        <div class="fade-up-element text-center mb-20 max-w-xl mx-auto">
            <h2 class="font-serif text-4xl md:text-5xl text-alpine-text mb-5 leading-tight">Le Sessioni</h2>
            <p class="text-[11px] uppercase tracking-widest text-alpine-muted font-bold">Tre momenti, tre energie diverse</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12">
            <!-- Alba -->
            <div class="fade-up-element group border border-alpine-border bg-white hover:border-alpine-gold hover:shadow-2xl transition-all duration-500">
                <div class="overflow-hidden aspect-3/2 bg-linear-to-b from-amber-100 to-orange-200 flex items-center justify-center">
                    <Sunrise class="w-16 h-16 text-alpine-gold/60 group-hover:scale-110 transition-transform duration-700" />
                </div>
                <div class="p-8 md:p-10">
                    <div class="flex items-center gap-2 mb-4">
                        <Sun class="w-4 h-4 text-alpine-gold" />
                        <span class="text-[10px] uppercase tracking-[0.2em] text-alpine-muted font-bold">Ore 07:00</span>
                    </div>
                    <h4 class="font-serif text-2xl text-alpine-text mb-2">Saluto al Sole</h4>
                    <p class="text-[10px] uppercase tracking-widest text-alpine-gold font-bold mb-4">Hatha Yoga · 60 min</p>
                    <p class="text-xs lg:text-sm text-alpine-muted font-light leading-relaxed">
                        Risveglia il corpo con una sequenza dolce e fluida mentre il sole 
                        illumina le vette. Posizioni classiche, respirazione consapevole 
                        e un'energia che ti accompagnerà per tutta la giornata.
                    </p>
                </div>
            </div>

            <!-- Pomeriggio -->
            <div class="fade-up-element group border border-alpine-border bg-white hover:border-alpine-gold hover:shadow-2xl transition-all duration-500">
                <div class="overflow-hidden aspect-3/2 bg-linear-to-b from-sky-100 to-blue-200 flex items-center justify-center">
                    <Wind class="w-16 h-16 text-alpine-gold/60 group-hover:scale-110 transition-transform duration-700" />
                </div>
                <div class="p-8 md:p-10">
                    <div class="flex items-center gap-2 mb-4">
                        <CloudSun class="w-4 h-4 text-alpine-gold" />
                        <span class="text-[10px] uppercase tracking-[0.2em] text-alpine-muted font-bold">Ore 15:00</span>
                    </div>
                    <h4 class="font-serif text-2xl text-alpine-text mb-2">Pranayama Alpino</h4>
                    <p class="text-[10px] uppercase tracking-widest text-alpine-gold font-bold mb-4">Breathwork · 60 min</p>
                    <p class="text-xs lg:text-sm text-alpine-muted font-light leading-relaxed">
                        Tecniche di respirazione avanzate nell'aria pura d'alta quota. 
                        L'ossigeno rarefatto amplifica ogni respiro, portando 
                        una chiarezza mentale che in pianura è difficile raggiungere.
                    </p>
                </div>
            </div>

            <!-- Tramonto -->
            <div class="fade-up-element group border border-alpine-border bg-white hover:border-alpine-gold hover:shadow-2xl transition-all duration-500">
                <div class="overflow-hidden aspect-3/2 bg-linear-to-b from-orange-100 to-rose-200 flex items-center justify-center">
                    <Moon class="w-16 h-16 text-alpine-gold/60 group-hover:scale-110 transition-transform duration-700" />
                </div>
                <div class="p-8 md:p-10">
                    <div class="flex items-center gap-2 mb-4">
                        <Moon class="w-4 h-4 text-alpine-gold" />
                        <span class="text-[10px] uppercase tracking-[0.2em] text-alpine-muted font-bold">Ore 18:30</span>
                    </div>
                    <h4 class="font-serif text-2xl text-alpine-text mb-2">Meditazione al Tramonto</h4>
                    <p class="text-[10px] uppercase tracking-widest text-alpine-gold font-bold mb-4">Yin Yoga & Meditazione · 60 min</p>
                    <p class="text-xs lg:text-sm text-alpine-muted font-light leading-relaxed">
                        Posizioni tenute a lungo, rilascio profondo delle tensioni 
                        e meditazione guidata mentre il sole tinge le vette di rosa. 
                        Il modo perfetto per chiudere la giornata e prepararsi a una notte di sonno ristoratore.
                    </p>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- ═══════════════════ RETREAT SETTIMANALE ═══════════════════ -->
<section class="bg-alpine-bg pb-32 px-6 max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20 items-center">
    <div class="fade-up-element order-2 md:order-1">
        <span class="text-[10px] uppercase tracking-[0.3em] text-alpine-gold font-bold mb-4 block">Immersione Totale</span>
        <h2 class="font-serif text-4xl text-alpine-text mb-6">Retreat<br/>Settimanale</h2>
        <p class="text-alpine-muted text-sm font-light leading-relaxed mb-6">
            Per chi desidera un'esperienza più profonda, organizziamo retreats settimanali 
            che combinano yoga, meditazione, escursioni consapevoli e alimentazione naturale. 
            Sette giorni per disconnettersi completamente e ritrovare l'equilibrio.
        </p>
        <ul class="space-y-3 text-xs text-alpine-muted font-light mb-8">
            <li class="flex items-start gap-3"><Star class="w-3.5 h-3.5 text-alpine-gold shrink-0 mt-0.5" /> 2 sessioni yoga al giorno (alba + tramonto)</li>
            <li class="flex items-start gap-3"><Star class="w-3.5 h-3.5 text-alpine-gold shrink-0 mt-0.5" /> Meditazione guidata quotidiana</li>
            <li class="flex items-start gap-3"><Star class="w-3.5 h-3.5 text-alpine-gold shrink-0 mt-0.5" /> Mindful hiking nei boschi di larici</li>
            <li class="flex items-start gap-3"><Star class="w-3.5 h-3.5 text-alpine-gold shrink-0 mt-0.5" /> Menù vegetariano a km 0</li>
            <li class="flex items-start gap-3"><Star class="w-3.5 h-3.5 text-alpine-gold shrink-0 mt-0.5" /> 1 trattamento spa incluso</li>
            <li class="flex items-start gap-3"><Star class="w-3.5 h-3.5 text-alpine-gold shrink-0 mt-0.5" /> Digital detox (volontario)</li>
        </ul>
        <div class="grid grid-cols-2 gap-6 pt-6 border-t border-alpine-border">
            <div class="flex items-center gap-3">
                <Clock class="w-5 h-5 text-alpine-gold shrink-0" />
                <span class="text-xs font-bold uppercase tracking-widest text-alpine-text">7 giorni</span>
            </div>
            <div class="flex items-center gap-3">
                <Users class="w-5 h-5 text-alpine-gold shrink-0" />
                <span class="text-xs font-bold uppercase tracking-widest text-alpine-text">Max 8 persone</span>
            </div>
        </div>
    </div>

    <div class="fade-up-element order-1 md:order-2 overflow-hidden bg-alpine-border shadow-xl">
        <img src="https://images.unsplash.com/photo-1545389336-cf090694435e?q=80&w=1500&auto=format&fit=crop" alt="Meditazione in montagna" class="w-full h-full object-cover img-elegant aspect-4/3 brightness-90" />
    </div>
</section>

<!-- ═══════════════════ BENEFICI ═══════════════════ -->
<section class="border-y border-alpine-border bg-white py-32 px-6">
    <div class="max-w-7xl mx-auto">
        <div class="fade-up-element text-center mb-20 max-w-xl mx-auto">
            <h2 class="font-serif text-4xl md:text-5xl text-alpine-text mb-5 leading-tight">Perché Yoga in Montagna</h2>
            <p class="text-[11px] uppercase tracking-widest text-alpine-muted font-bold">I benefici dell'alta quota sulla pratica</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            <div class="fade-up-element text-center p-10">
                <Wind class="w-8 h-8 text-alpine-gold mx-auto mb-6 stroke-[1.5px]" />
                <h4 class="font-serif text-lg text-alpine-text mb-3">Aria Purissima</h4>
                <p class="text-xs text-alpine-muted font-light leading-relaxed">
                    L'aria d'alta quota è priva di inquinamento. Ogni respiro è più profondo e consapevole.
                </p>
            </div>
            <div class="fade-up-element text-center p-10">
                <Sparkles class="w-8 h-8 text-alpine-gold mx-auto mb-6 stroke-[1.5px]" />
                <h4 class="font-serif text-lg text-alpine-text mb-3">Silenzio Totale</h4>
                <p class="text-xs text-alpine-muted font-light leading-relaxed">
                    Nessun rumore urbano. Solo il vento, gli uccelli e il tuo respiro. La meditazione diventa naturale.
                </p>
            </div>
            <div class="fade-up-element text-center p-10">
                <Leaf class="w-8 h-8 text-alpine-gold mx-auto mb-6 stroke-[1.5px]" />
                <h4 class="font-serif text-lg text-alpine-text mb-3">Connessione Naturale</h4>
                <p class="text-xs text-alpine-muted font-light leading-relaxed">
                    Praticare circondati dalla natura amplifica la sensazione di radicamento e presenza.
                </p>
            </div>
            <div class="fade-up-element text-center p-10">
                <Heart class="w-8 h-8 text-alpine-gold mx-auto mb-6 stroke-[1.5px]" />
                <h4 class="font-serif text-lg text-alpine-text mb-3">Recupero Muscolare</h4>
                <p class="text-xs text-alpine-muted font-light leading-relaxed">
                    Dopo una giornata di sci o trekking, lo yoga rilascia le tensioni e accelera il recupero.
                </p>
            </div>
        </div>
    </div>
</section>

<!-- ═══════════════════ L'INSEGNANTE ═══════════════════ -->
<section class="bg-alpine-bg py-32 px-6">
    <div class="max-w-5xl mx-auto">
        <div class="fade-up-element text-center mb-20 max-w-xl mx-auto">
            <h2 class="font-serif text-4xl md:text-5xl text-alpine-text mb-5 leading-tight">La Tua Guida</h2>
            <p class="text-[11px] uppercase tracking-widest text-alpine-muted font-bold">Professionalità e passione</p>
        </div>

        <div class="fade-up-element bg-white p-12 md:p-16 border border-alpine-border max-w-3xl mx-auto">
            <div class="text-center">
                <Flower class="w-10 h-10 text-alpine-gold mx-auto mb-8 stroke-[1.5px]" />
                <p class="text-alpine-muted text-sm font-light leading-relaxed italic mb-8 max-w-xl mx-auto">
                    "La montagna ti insegna ciò che nessun maestro può: 
                    che sei piccolo, che sei forte, e che il silenzio 
                    è il suono più potente che esista."
                </p>
                <p class="text-xs text-alpine-muted font-light leading-relaxed max-w-xl mx-auto mb-8">
                    La nostra insegnante è certificata Yoga Alliance RYT-500 con specializzazione 
                    in yoga di montagna e meditazione mindfulness. Anni di pratica nelle Alpi 
                    le hanno insegnato ad adattare ogni sessione all'altitudine, al clima 
                    e all'energia del gruppo. Ogni lezione è unica, proprio come ogni giornata in montagna.
                </p>
                <div class="flex flex-wrap justify-center gap-4">
                    <span class="text-[9px] uppercase tracking-[0.2em] font-bold text-alpine-gold border border-alpine-gold/30 px-4 py-2">RYT-500</span>
                    <span class="text-[9px] uppercase tracking-[0.2em] font-bold text-alpine-gold border border-alpine-gold/30 px-4 py-2">Hatha</span>
                    <span class="text-[9px] uppercase tracking-[0.2em] font-bold text-alpine-gold border border-alpine-gold/30 px-4 py-2">Yin</span>
                    <span class="text-[9px] uppercase tracking-[0.2em] font-bold text-alpine-gold border border-alpine-gold/30 px-4 py-2">Pranayama</span>
                    <span class="text-[9px] uppercase tracking-[0.2em] font-bold text-alpine-gold border border-alpine-gold/30 px-4 py-2">Mindfulness</span>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- ═══════════════════ INFO PRATICHE ═══════════════════ -->
<section class="bg-alpine-bg pb-32 px-6">
    <div class="max-w-5xl mx-auto">
        <div class="fade-up-element text-center mb-20 max-w-xl mx-auto">
            <h2 class="font-serif text-4xl md:text-5xl text-alpine-text mb-5 leading-tight">Info Pratiche</h2>
            <p class="text-[11px] uppercase tracking-widest text-alpine-muted font-bold">Tutto quello che devi sapere</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="fade-up-element bg-white p-10 border border-alpine-border">
                <h4 class="font-serif text-xl text-alpine-text mb-6 flex items-center gap-3">
                    <Clock class="w-5 h-5 text-alpine-gold" />
                    Orari & Sessioni
                </h4>
                <div class="space-y-4 text-sm text-alpine-muted font-light">
                    <div class="flex justify-between items-center border-b border-alpine-border pb-3">
                        <span>Saluto al Sole</span>
                        <span class="font-bold text-alpine-text">07:00 – 08:00</span>
                    </div>
                    <div class="flex justify-between items-center border-b border-alpine-border pb-3">
                        <span>Pranayama Alpino</span>
                        <span class="font-bold text-alpine-text">15:00 – 16:00</span>
                    </div>
                    <div class="flex justify-between items-center border-b border-alpine-border pb-3">
                        <span>Meditazione al Tramonto</span>
                        <span class="font-bold text-alpine-text">18:30 – 19:30</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span>Disponibilità</span>
                        <span class="font-bold text-alpine-text">Tutti i giorni</span>
                    </div>
                </div>
            </div>

            <div class="fade-up-element bg-white p-10 border border-alpine-border">
                <h4 class="font-serif text-xl text-alpine-text mb-6 flex items-center gap-3">
                    <ShieldCheck class="w-5 h-5 text-alpine-gold" />
                    Da Sapere
                </h4>
                <div class="space-y-4 text-sm text-alpine-muted font-light">
                    <div class="flex justify-between items-center border-b border-alpine-border pb-3">
                        <span>Livello richiesto</span>
                        <span class="font-bold text-alpine-text">Nessuno</span>
                    </div>
                    <div class="flex justify-between items-center border-b border-alpine-border pb-3">
                        <span>Tappetino</span>
                        <span class="font-bold text-alpine-text">Fornito dall'hotel</span>
                    </div>
                    <div class="flex justify-between items-center border-b border-alpine-border pb-3">
                        <span>Prenotazione</span>
                        <span class="font-bold text-alpine-text">Alla reception (entro sera prima)</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span>Costo</span>
                        <span class="font-bold text-alpine-text">Incluso nel soggiorno</span>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- ═══════════════════ CTA ═══════════════════ -->
<section class="bg-[#0a0a0a] py-32 px-6">
    <div class="max-w-3xl mx-auto text-center">
        <div class="fade-up-element">
            <h2 class="font-serif text-4xl text-white mb-6">Trova il Tuo Equilibrio</h2>
            <p class="text-white/50 text-sm font-light leading-relaxed max-w-lg mx-auto mb-12">
                L'aria sottile, il silenzio delle vette, il primo raggio di sole sulle Alpi. 
                Non è solo yoga, è un'esperienza che ti cambia.
            </p>
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                <a href="/wellness" class="inline-flex items-center justify-center gap-3 bg-white text-alpine-text px-10 py-4 text-[11px] tracking-[0.2em] uppercase font-bold hover:bg-alpine-gold hover:text-white transition-all duration-300">
                    Scopri il Wellness <ArrowRight class="w-4 h-4" />
                </a>
                <a href="/wellness/massaggi" class="inline-flex items-center justify-center gap-3 border border-white/30 text-white px-10 py-4 text-[11px] tracking-[0.2em] uppercase font-bold hover:border-white hover:bg-white/10 transition-all duration-300">
                    Massaggi Post-Yoga <ArrowRight class="w-4 h-4" />
                </a>
            </div>
        </div>
    </div>
</section>
